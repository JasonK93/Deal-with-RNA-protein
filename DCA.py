...
def Compute_True_Frequencies(align,M,N,q,theta):
    ...
    for l in xrange(0, M):
        for i in xrange(0, N-1):
            for j in xrange(i+1,N):
                Pij_true[i, j, int(align[l,i]-1), int(align[l,j]-1)] = Pij_true[i,j,int(align[l,i]-1),int(align[l,j]-1)] + W[l]
                Pij_true[j,i,int(align[l,j]-1),int(align[l,i]-1)] = Pij_true[i,j,int(align[l,i]-1),int(align[l,j]-1)]
...
def ReturnW(C, i, j, q):
    W = np.ones((q,q))
    ##########
    # W[0:q-1,0:q-1] = math.exp(-C[mapkey(i,0,q),mapkey(j,0,q)])
